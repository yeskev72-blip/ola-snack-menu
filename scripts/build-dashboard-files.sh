...
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/analyze-meal" "$TMP/delete-account" "$TMP/create-checkout" "$TMP/cinetpay-webhook"
...
# --- Edge Functions ----------------------------------------------------------
for fn in analyze-meal delete-account create-checkout cinetpay-webhook; do
